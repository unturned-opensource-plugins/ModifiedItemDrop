...
namespace FFEmqo.ModifiedItemDrop.Configuration
{
    public class DropRuleSet
    {
        ...
        {
            // 双重检查锁定模式
            if (_regionChanceMap != null && _itemChanceMap != null && _clothingRuleMap != null)
            {
                return;
            }
            ...
            lock (_cacheLock)
            {
                ...
                var regionMap = new Dictionary<string, double>(StringComparer.OrdinalIgnoreCase);
                if (RegionChances != null)
                {
                    foreach (var entry in RegionChances)
                    {
                        ...
                        regionMap[entry.Region.Trim()] = UtilityHelper.ClampChance(entry.Chance);
                    }
                }

                var itemMap = new Dictionary<ushort, double>();
                if (CustomItemChances != null)
                {
                    foreach (var entry in CustomItemChances)
                    {
                        ...
                        itemMap[entry.ItemID] = UtilityHelper.ClampChance(entry.Chance);
                    }
                }

                var clothingMap = new Dictionary<SlotType, ClothingSlotRule>();
                if (ClothingRules != null)
                {
                    foreach (var rule in ClothingRules)
                    {
                ...
                // 原子性地设置所有缓存
                _regionChanceMap = regionMap;
                _itemChanceMap = itemMap;
                _clothingRuleMap = clothingMap;
            }
        }
